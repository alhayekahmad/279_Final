...
Address::Address(string st, string state, string zip, string country, string city) {
	this->st_address = st;
	this->state = state;
	this->zip_code = zip;
...
vector<string> Address::reString() {

	vector<string> ad;
	ad.push_back(st_address);
	ad.push_back(state);
	ad.push_back(zip_code);
	ad.push_back(country);
	ad.push_back(city);
	return ad;
}
